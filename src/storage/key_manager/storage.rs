...
#[derive(Clone)]
pub struct TransactionKeyManagerWalletStorage<TWalletStorage: WalletStorage + Clone> {
    database_connection: Arc<TWalletStorage>,
    cipher: Arc<RwLock<XChaCha20Poly1305>>,
    wallet_id: u32,
}

#[allow(unused)]
impl<TWalletStorage: WalletStorage + Clone> TransactionKeyManagerWalletStorage<TWalletStorage> {
    ...
    pub fn new(
        database_connection: TWalletStorage,
...
impl<TWalletStorage: WalletStorage + Clone> TransactionKeyManagerBackend
    for TransactionKeyManagerWalletStorage<TWalletStorage>
{
    fn get_key_manager(
        &self,
        branch: &str,
...
mod test {
    ...
    async fn setup_db() -> TransactionKeyManagerWalletStorage<SqliteStorage> {
        ...
        TransactionKeyManagerWalletStorage::new(db, cipher, wallet_id)
    }
    ...
    async fn test_build_and_cipher_consistency() {
        ...
        let km_db2 = TransactionKeyManagerWalletStorage::build(db.clone(), wallet_id)
            .await
            .unwrap();
    ...
    async fn test_build_non_existent_wallet() {
        ...
        let result = TransactionKeyManagerWalletStorage::build(db.clone(), 999).await;
