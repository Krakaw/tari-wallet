...
mod test {
    ...
    async fn test_build_and_cipher_consistency() {
        ...
        let km_db1 = TransactionKeyManagerSqliteDatabase::build(db.clone(), wallet_id)
            .await
            .unwrap();

        // Create the second key manager instance for the same wallet
        let km_db2 = TransactionKeyManagerSqliteDatabase::build(db.clone(), wallet_id)
            .await
            .unwrap();
    ...
    async fn test_build_non_existent_wallet() {
        ...
        let result = TransactionKeyManagerSqliteDatabase::build(db.clone(), 999).await;
